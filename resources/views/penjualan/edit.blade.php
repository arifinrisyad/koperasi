                    ...
                    <form action="{{ route('penjualan.update', $penjualan->id) }}" method="POST">
                            ...
                            <select name="barang_dijuals_id" id="barang_dijuals_id" 
                                ...
                                @foreach($barangs as $barang)
                                    <option value="{{ $barang->id }}" 
                                            data-stok="{{ $barang->stok + ($barang->id == $penjualan->barang_dijuals_id ? $penjualan->jumlah : 0) }}"
                                            data-harga="{{ $barang->harga_jual }}"
                                            {{ $barang->id == $penjualan->barang_dijuals_id ? 'selected' : '' }}>
                                        {{ $barang->nama_barang }} (Stok: {{ $barang->stok + ($barang->id == $penjualan->barang_dijuals_id ? $penjualan->jumlah : 0) }}) - 
                        ...
                        <div class="form-group mb-3">
                            <label for="jumlah" class="form-label">
                                <i class="bi bi-stack me-2"></i> Jumlah
                            </label>
                            <input type="number" name="jumlah" id="jumlah" 
                                   class="form-control @error('jumlah') is-invalid @enderror" 
                                   required min="1" 
                                   value="{{ old('jumlah', $penjualan->jumlah) }}"
                                   onchange="checkStock(this.value)">
                            @error('jumlah')
                                <div class="invalid-feedback">
                                    <i class="bi bi-info-circle me-2"></i> {{ $message }}
                                </div>
                        ...
                        <div class="form-group mb-3">
                            <label for="total_harga" class="form-label">
                                <i class="bi bi-cash-stack me-2"></i> Total Harga
                            </label>
                        ...
                        <div class="form-group text-end">
                            <button type="submit" class="btn btn-primary" id="submitBtn">
                                <i class="bi bi-save me-2"></i> Update Penjualan
                            </button>
                        </div>
                    </form>
                </div>
